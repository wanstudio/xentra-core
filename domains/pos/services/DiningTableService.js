...
class DiningTableService {
  static initializeBranchLayout(branchId, templateId = 'template_01') {
    ...
    try {
      repository.upsertBranchLayout({
        layoutId,
        branchId,
        canvasConfig,
        sectionsConfig,
        nonTableObjectsConfig: nonTableConfig,
        createdAt: now,
        updatedAt: now
      });
      ...
      if (template && Array.isArray(template.tables)) {
        for (const t of template.tables) {
          ...
          repository.ensureTableState({
            tableId,
            operationalState: t.initial_state || 'available',
            updatedAt: now
          });
        }
      }
  ...
  static setTableBlockedState(tableId, isBlocked, reason = '') {
    ...
    repository.updateTableState({
      ...
      currentSessionId: null,
      notes: reason || (isBlocked ? 'Blocked by staff' : 'Unblocked by staff'),
      updatedAt: now
    });

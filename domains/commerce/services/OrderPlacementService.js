...
class OrderPlacementService {
  static async submitOrder({
    ...
    if (effectiveOrderType === 'dine_in') {
      const diningContext = DiningTableService.prepareDineInOrderContext({
        branch_id,
        order_channel,
      ...
      if (!diningContext.valid) {
        return {
          success: false,
          status: diningContext.status,
          errors: diningContext.errors
        };
      }

      effectiveDiningSessionId = diningContext.dining_session_id;
      resolvedTableNumber = diningContext.table_number;
      resolvedTableIds = diningContext.table_ids;
    }
    ...
    try {
      ...
      if (shift_id && effectivePaymentMethod === 'cash') {
        const shiftUpdateRes = posShiftRepository.incrementCashSales({ shiftId: shift_id, branchId: branch_id, amount: grandTotal });
    ...
    await events.EventBus.publish({
      ...
      producer: 'commerce',
      payload: { order_id: orderId, order_number: orderNumber, brand_id, branch_id, subtotal, delivery_fee, grand_total: grandTotal, items: verifiedItems, customer },
      context: { correlation_id: trace_context.correlation_id, causation_id: orderId }
    });
  ...
  static deductStockForSettledOrder(orderId, { dbTransactionProvided = false } = {}) {
    ...
    if (!items || items.length === 0) return { success: true, deducted_items: [] };

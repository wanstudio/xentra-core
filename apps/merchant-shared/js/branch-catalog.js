/**
 * XENTRA CORE — MERCHANT SHARED BRANCH CATALOG
 *
 * Branch catalog adoption, branch categories and branch-product override
 * shared by the Owner dashboard and the Branch Manager surface.
 * Must be loaded AFTER merchant-shared/js/shared.js.
 *
 * Surface-specific behaviour is injected through hooks (setHooks):
 *   getBranches()  -> branch list for the catalog modal title (Owner)
 *   refreshBMMenu() -> refresh the Branch Manager menu after a mutation
 * State that both surfaces must share is exposed on XentraBranchCatalog.state.
 */
(function () {
  'use strict';
  ...
  window.XentraBranchCatalog = {
    ...
    renderInlineCategoriesBar: renderInlineCategoriesBar,
    saveBranchCategoryOrder: saveBranchCategoryOrder,
    renderInlineAdoptedProducts: renderInlineAdoptedProducts,
    renderInlineAvailableProducts: renderInlineAvailableProducts,
